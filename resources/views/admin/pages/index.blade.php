    ...
    <table class="table">
        ...
        @if($authRole->can(PermissionEnum::INVOICES_VIEW))
            <tr>
                ...
                <td>
                    ...
                    <div>
                        К счёту можно прикреплять платежи - они автоматически будут распределены по транзакциям в отметку "оплачено". <br>
                        ...
                        Расчёт пунктов "оплачено" для транзакций и счетов всегда рассчитывается автоматически.<br><br>
        ...
        @if($authRole->can(PermissionEnum::PAYMENTS_VIEW))
            <tr>
                ...
                <td>
                    Обработка платежей, полученных через <a href="{{ route(RouteNames::PAYMENT) }}">форму обращений</a>, и привязка их к счетам.
                </td>
            </tr>
        @endif
        @if($authRole->can(PermissionEnum::COUNTERS_VIEW))
            <tr>
                ...
                <td>
                    ...
                    <a href="{{ route(RouteNames::COUNTER) }}">форму обращений</a> и из личного кабинета члена СНТ.<br>
                    При подтверждении показаний для указанного участка будет найден или создан счёт в текущем периоде,
                    к нему будет добавлена транзакция с типом <b>"{{ ServiceTypeEnum::ELECTRIC_TARIFF->name() }}"</b> и рассчитаной стоимостью (Текущий тариф электроэнергии текущего периода * Разницу показаний) .<br>
                </td>
            </tr>
        @endif
        </tbody>
