...
export function useInvoicesBlock () {
    ...
    const recalcAction = async () => {
        ...
        if (!confirm('Пересчитать все счета периода? Нераспределённые средства участов будут автоматически распределены на все неоплаченные услуги. Для детальной настройки пользуйтесь ручным расчётом.')) {
            return;
        }
        try {
