...
readonly class HistoryChangesDecorator
{
    ...
    public function getActionEventText(): string
    {
        ...
        return match ($this->logData->getEvent()) {
            ...
            default       => (string) $this->logData->getText(),
        };
    }
    ...
    public function getPrimaryUrl(): ?string
    {
        ...
        if ($h->getType() === HistoryType::COUNTER) {
            $accountId = $this->counterService->getById((int) $h->getPrimaryId())?->getAccountId();
        }
        ...
        if ($h->getType() === HistoryType::INVOICE) {
            if ($h->getPrimaryId() === null) {
                switch ($h->getReferenceType()) {
                    ...
                    {
                        $id = $this->paymentService->getById($h->getReferenceId())?->getInvoiceId();
                        break;
                    }
                    case HistoryType::CLAIM:
                    {
                        $id = $this->claimService->getById($h->getReferenceId())?->getInvoiceId();
                        break;
                    }
                    default:
                    {
                ...
                if ($id) {
                    $h->setPrimaryId($id);
                    $this->historyChangesService->save($h);
                }
                else {
                    return null;
                }
