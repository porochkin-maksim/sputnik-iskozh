...
- В этой среде запрещено полагаться на системный `php`, `composer` и локальное окружение хоста.
- Все проектные PHP/Composer/Artisan/PHPUnit команды запускай через Docker/Sail: `make artisan ...`,
  `make composer ...`, `make tests`, `./vendor/bin/sail php ...`.
...
- Если код использует fluent-chain после `new`, не добавляй лишние скобки вокруг `new ...()`, если выражение и так
  валидно.
- Для внешних библиотек используй один канонический project adapter/service на технологическую область, а не
  инстанцируй библиотечные объекты в произвольных местах.
...
- PHPUnit в `tests/`
- Перед отправкой кода убедись, что `make tests` или точечный `make artisan test --filter ...` проходит
