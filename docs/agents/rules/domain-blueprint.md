...
- Job должен оставаться thin и вызывать application-layer orchestration, а не содержать длинную бизнес-логику.
- Если job использует блокировки или дедупликацию, эта ответственность должна быть вынесена в отдельный adapter/trait/service.
- Имена методов и интерфейсов для job должны быть однозначными и отражать роль, а не legacy-реализацию.
...
- Если аргументов много или сценарий вызывается из разных transport-слоёв, используй readonly `*Input` рядом с command.
- `*Command` является исполняемым use case с зависимостями и методом `execute()`.
- Не создавать `*Command` как data object и отдельный `*Handler` как исполнитель.
- Не называть application input object `*Request`, `*Dto` или `*Bag`.
