...
class ReceiptController extends Controller
{
    ...
    public function makeForBlank(DefaultRequest $request): BinaryFileResponse
    {
        ...
        $sntAccounting = $this->optionService->getByType(OptionEnum::SNT_ACCOUNTING)->getData();
        $chairmanInfo  = $this->optionService->getByType(OptionEnum::CHAIRMAN_INFO)->getData();

        Pdf::view('exports.documents.receipt.receipt', compact('period', 'services', 'sntAccounting', 'chairmanInfo'))
            ->format('a4')
            ->portrait()
            ->save($tempFile)
        ;
    ...
    public function makeByInvoiceId(mixed $id): BinaryFileResponse
    {
        ...
        Pdf::view('exports.documents.receipt.receipt', compact('invoice', 'sntAccounting', 'chairmanInfo'))
            ->format('a4')
            ->portrait()
            ->save($tempFile)
        ;
